...
void sumarComando(string in, Arbol &a, int i) {
     if(cantidadPalabras(in) != 4)
        ...
        if(!ExistePolinomio(a, sumando1)) {
            printf("\nEl nombre del polinomio ingresado no se encuentra: ");
            print(sumando1);
            return;
        }
...
void multiplicarComando(string in, Arbol &a, int i) {
     if(cantidadPalabras(in) != 4)
        ...
        if(!ExistePolinomio(a, mult1)) {
            printf("El nombre del polinomio ingresado no se encuentra: ");
            print(mult1);
            return;
        }
...
void evaluarComando (string input, Arbol a){
    ...
    printf("El resultado de evaluar el polinomio con %d es:\t%d",numero ,resultado);
